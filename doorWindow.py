...
class doorWindow(QWidget):
    def __init__(self):
        ...
        background.setStyleSheet('QWidget{border-image:url(./static/doorimg/图层 3.png);}')
        perimgLayout = QVBoxLayout()
        perimg = QWidget()
        perimg.setFixedHeight(815)
        perimg.setFixedWidth(580)
        perimg.setStyleSheet("QWidget{border-image:url(./static/doorimg/图层 4.png);}")
        perimgLayout.addWidget(perimg)
        background.setLayout(perimgLayout)
        studentDataLayout.addWidget(background)
        self.seat = QLabel()
        seatimg = PyQt5.QtGui.QPixmap('./static/doorimg/框.png')
        ...
        for i in range(3):
            ...
            line = QLabel()
            lineimg = PyQt5.QtGui.QPixmap('./static/doorimg/线.png')
        ...
        img = PyQt5.QtGui.QPixmap('./static/doorimg/图层 4.4.png')
        titleRightImg.setPixmap(img)
        titleLayout.addWidget(titleRightImg)
        titleImg = QLabel()
        img = PyQt5.QtGui.QPixmap('./static/doorimg/信息学院智能学工.png')
        
        titleImg.setPixmap(img)
        titleLayout.addWidget(titleImg)
        titleLeftImg = QLabel()
        img = PyQt5.QtGui.QPixmap('./static/doorimg/图层 4.3.png')
